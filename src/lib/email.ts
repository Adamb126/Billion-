...
//
// SMTP transport is shared by all studios (one set of credentials), but the
// studio name and "from" address come from the studio itself.
// When SMTP isn't configured (local dev), we log the email to the console so
// the flow is still fully testable.
...
export async function sendBookingConfirmation(
  ...
  const subject = `Booking confirmed — ${details.serviceName} at ${details.studioName}`;
  ...
  const text = [
    ...
    ``,
    `Your booking at ${details.studioName} is confirmed. We look forward to seeing you!`,
    ``,
  ...
  const html = `
    <div style="font-family: -apple-system, Segoe UI, Roboto, sans-serif; max-width: 480px; margin: 0 auto; color: #0f172a;">
      ...
      <p>Your booking at <strong>${escapeHtml(details.studioName)}</strong> is confirmed. We look forward to seeing you!</p>
      ...
      <p>See you soon,<br/>${escapeHtml(details.studioName)}</p>
    </div>`;
